...
-export([make_relup/3, make_relup/4]).
...
%% ----------------------------------------------------------------
make_relup(TopRelFile, BaseUpRelDcs, BaseDnRelDcs) ->
    make_relup(TopRelFile, BaseUpRelDcs, BaseDnRelDcs, []).
make_relup(TopRelFile, BaseUpRelDcs, BaseDnRelDcs, Opts) ->
    io:format("make_relup(~p, ~p, ~p, ~p)\n",
              [TopRelFile, BaseUpRelDcs, BaseDnRelDcs, Opts]),
...
get_script_from_appup(Mode, TopApp, BaseVsn, Ws, RUs) ->
    ...
    case systools_lib:read_term(FName) of
        {ok, {TopVsn, UpVsnRUs, DnVsnRUs}} ->
           VsnRUs = case Mode of
                         up ->
                             UpVsnRUs;
                         dn ->
           ...
           io:format("OK = get_script_from_appup ~s --> ~p\n",[
                                                               FName, {VsnRUs, TopVsn}]),
           Ws1 = if
        ...
        X ->
           %% if file missing, inject a fake "restart application" appup
           io:format("ERR get_script_from_appup ~s ~p\n~p\n~p // ~p\n",[FName,X,TopApp,BaseVsn,Mode]),
           %throw({error, ?MODULE, {file_problem,
           %{FName, X}}})
           FakeRUs = [{restart_application, TopApp#application.name}],
           %Warning = {missing_appup, TopApp#application.name},
           {RUs ++ [FakeRUs], Ws}
    end.
